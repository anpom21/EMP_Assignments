...
*/
{
    ...
    while (1)
    {
        ...
        // Determine production frequency based on liquid base
        switch (liquid_base)
        {
        case MILK:
        ...
        // Determine production time based on flavour and start production.
        switch (chosen_flavour)
        {
        case VANILLA:
            production_time = churning_time_vanilla * icecream_amount;
            finished = led_flashing(LED_YELLOW, 2 * production_frequency, production_time);
            break;
        case CHOCOLATE:
            production_time = churning_time_chocolate * icecream_amount;
            finished = led_flashing(LED_GREEN, 2 * production_frequency, production_time);
            break;
        case STRAWBERRY:
            production_time = churning_time_strawberry * icecream_amount;
